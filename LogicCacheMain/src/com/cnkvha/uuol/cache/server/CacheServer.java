...
import com.cnkvha.uuol.cache.server.utils.chunk.ChunkLoader;
...
public class CacheServer {
	...
	private void run(){
		...
		chunkLoader = new ChunkLoader(GlobalConfiguration.CHUNK_LOADER_THREADPOOL_SIZE);
	...
	public void sendPacketTo(List<String> targetClusters, CacheAbstractMessage msg, String component){
		...
		if(data == null) return;
		try {
			gms.getGroupHandle().sendMessage(targetClusters, component.equals("") ? null : component , data);
		} catch (Exception e) {
			e.printStackTrace();
			getLogger().error("Error sending packet! ");
		}
	...
	public void sendPacketTo(String[] targetClusters, CacheAbstractMessage msg, String component){
		sendPacketTo(Arrays.asList(targetClusters), msg, component);
	}
